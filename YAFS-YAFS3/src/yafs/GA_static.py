...
class GA:
    ...
    """
    def get_fitness_score(self, population, nodes_ram, modules_ram, msg, G,bw,nx, shift_delay, old_placement, sim_parameters):
        #print("在get_fitness_score之后")
        ...
        for gen in generationSelect:
            ...
            '''
            这里需要执行我的new_simulator类
            (self,t, population, selectorPath, apps, folder_results,new_palcement_list, untiltime, old_placement_json, shift_delay):
            t: Topology ?应该是t还是G
            ...
            new_placement_list:gen 正确

            untiltime:
            old_placement_json:

            shift_delay:shift_delay 正确
    ...
    def select(self, population, nodes_ram, modules_ram, msg, G,bw,nx, pop_flag = False):
        ...
        for gen in generationSelect:
            ...
            param repMSG: record the MSG size of each App's path
            param repBW_big: record the bandwidth of each App's path
            param repPR: the copy one of repBW_big
            param fitness_store: store each generation's fitness score
            """
            counter = 0
            app_lat = []
            ...
            fitness_scores.append(sum(rep_fit))

        #print("fitness 测试")
        #print(fitness_scores)
        
        """
        上面计算了每一个estimated latency，然后我需要检测这些link有没有shift latency，如果有的话需要加上。
        而且我还需要计算从最初的placement转到这个placement的adaptation cost。
        """

        """
        上面这一段计算了每一个generation的fitness score
        """
        ...
        #fitness calculation for best fit
            ...
            return val, generationSelect[idx], fitness_scores[idx]
    """
    Function:
    ...
    def run(self, nodes, apps, modules, pop_size, links, nodes_ram, modules_ram,msg,nx,G,bw,gen_size):
        ...
        while converge < threshold:
            ...
            fitness = self.select(population, nodes_ram, modules_ram, msg, G,bw,nx)
