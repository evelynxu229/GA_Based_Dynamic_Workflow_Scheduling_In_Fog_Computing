...
class Sim:
    ...
    **Main variables to coordinate with algorithm:**
                ...
                try:
                   # transmit = size_bits / (self.topology.get_edge(link)[Topology.LINK_BW] * 1000000.0)  # MBITS!
                    ...
                    self.metrics.insert_link(
                        {"id":message.id,"type": self.LINK_METRIC,"src":link[0],"dst":link[1],"app":message.app_name,"latency":latency_msg_link,"message": message.name,"ctime":self.env.now,"size":message.bytes,"buffer":self.network_pump,"path":message.path,"shiftime":shift_time})
    ...
    #这个是用于deploy_module的，也就是要是我有了新的deployment，那么我就检查所有的deployment，首先undeployed_all_modules, 然后再用这个函数deploy所有的module
    ...
    def run(self, until, show_progress_monitor=False, test_initial_deploy=False):
        ...
        """
        #将__network_process进程加入到仿真环境中，但是并没有开始执行
        ...
        for pop in self.population_policy.items():
            #print("deploying sources in the topo")
        ...
        for place in self.placement_policy.items():
            #print("creating init deploy of services")
