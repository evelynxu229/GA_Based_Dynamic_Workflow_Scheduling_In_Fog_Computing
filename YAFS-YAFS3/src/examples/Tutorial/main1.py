...
def create_application():
    ...
    """
    #message between the sensor and the module on the node
    m_a = Message("M.A", "Sensor", "ServiceA", instructions=20*10^6, bytes=1000)

    #message between the module on the node and the acutator
    ...
    #the message between the sensor and the node is dynamically generated
    ...
    # MODULE SERVICES
    #fractional_selectivity: distribution functions that modules use to create and send the different messages
...
def main(simulated_time):
    ...
    """
    #creating the topology from the function create_json_topology
    t = Topology()
    ...
    APPLICATION
    """
    #creating the application from the function create_application
    app = create_application()
    print("application", app)
    """
    PLACEMENT algorithm
    This implementation locates the services of the application
    in the cheapest cloud regardless of where the sources "sensors" or sinks "acutators" are located.
    """
    placement = CloudPlacement("onCloud") # it defines the deployed rules: module-device

    #--------------------- what is scale services? --------------------------------
    placement.scaleService({"ServiceA": 1})

    """
    POPULATION algorithm
    
    a statical creation of requests. For each message of a pure service source (or sensor),
    it will have a generation control associated to it.
    """
